...
let textColor = 'black'; // Variable global para el color del texto

document.addEventListener('themeChanged', (event) => {
    textColor = event.detail.darkMode ? 'white' : 'black';
    // Re-renderiza el texto con el nuevo color de texto
    showGameOver();
    showStartGameText();
    score.draw();
});
...
const PLAYER_WIDTH = 88 / 1.5; // 58.67
const PLAYER_HEIGHT = 94 / 1.5; // 62.67
...
function createSprites() {
  ...
  const cactiImages = CACTI_CONFIG.map(cactus => {
    ...
    return { image: image, width: cactus.width * scaleRatio, height: cactus.height * scaleRatio };
  });
